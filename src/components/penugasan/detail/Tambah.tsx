...
const AddPenugasan: React.FC<AddPenugasanProps> = ({ skNumber, idDaops, idSuratTugas, onClose, onSuccess }) => {
    const [formData, setFormData] = useState<SuratTugasTeamMemberData>({
        id: `new_${Date.now()}`,
        r_surat_tugas_id: idSuratTugas,
        r_role_id: "4",
        isActive: true,
        name: '', email: '', phoneNumber: '', registrationNumber: '',
        startDate: '', endDate: '', daops: '', posko: '',
        kode_wilayah: '', id_daerah_patroli: '',
        groupMembers: [], organization: '', id_daops: idDaops,
        no_registrasi: ''
    });
    ...
    const [jenisPatroli, setJenisPatroli] = useState<string>('mandiri');
    ...
    const roleOptions = [
        { value: "4", label: "Ketua" }, { value: "5", label: "Anggota" }, { value: "6", label: "TNI" },
        { value: "7", label: "Polri" }, { value: "8", label: "MPA" }, { value: "10", label: "Pemda" },
        { value: "11", label: "Swasta" }, { value: "12", label: "Masyarakat" }
    ];
    ...
    const loadUserOptions = useCallback(
        debounce((inputValue: string, callback: (options: (EmailOption & { data: UserData })[]) => void) => {
            if (inputValue.trim().length < 2) { callback([]); return; }
            getAllUsersPersonil(inputValue.trim())
                .then(users => {
                    const options = users.map(user => ({
                        value: user.email, label: `${user.name} (${user.email})`, data: user
                    }));
                    callback(options);
                }).catch(() => callback([]));
        }, 300), []
            ...
            getAllKelurahan2(1, 1000, inputValue.trim())
                .then(data => {
                    ...
                    const options: KelurahanOption[] = filteredData.map((item: any) => ({
                        value: item.id, label: `${item.kode} - ${item.nama}`,
    ...
    const handleMainEmailSelect = (option: (EmailOption & { data: UserData }) | null) => {
        ...
        setFormData(prev => ({
            ...prev, email: user?.email || '', name: user?.name || '',
            registrationNumber: user?.registrationNumber || '', phoneNumber: user?.phoneNumber?.toString() || ''
        }));
    };
    ...
    const handleGroupEmailSelect = (option: (EmailOption & { data: UserData }) | null, groupIndex: number) => {
        ...
        updatedMembers[groupIndex] = {
            ...updatedMembers[groupIndex], email: user?.email || '',
    ...
    const handleAddMember = () => {
        const newMember: SuratTugasTeamMemberData = {
            id: `new_${Date.now()}`, r_surat_tugas_id: idSuratTugas, r_role_id: "5", isActive: true,
    ...
    const handleKelurahanSelect = (option: KelurahanOption | null) => {
        ...
        setFormData(prev => ({
            ...prev, posko: option?.data.nama || '', kode_wilayah: option?.data.kode || '',
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!formData.email || !formData.id_daerah_patroli || !formData.startDate || !formData.endDate) {
            Swal.fire('Data Tidak Lengkap', 'Pastikan Ketua, Desa/Posko, dan Periode Tanggal sudah terisi.', 'warning');
            return;
        }
        setLoading(true);
        const apiCalls: Promise<any>[] = [];
        const allMembers = [formData, ...groupMembersData];
        for (const member of allMembers) {
            if (!member.email) continue;
            const payload = {
                tanggal_awal: formData.startDate?.replace(/-/g, '/'),
                tanggal_akhir: formData.endDate?.replace(/-/g, '/'),
                id_roles: member.r_role_id, email: member.email,
                kode_wilayah: formData.kode_wilayah?.replace(/\./g, ''),
                id_daops: idDaops, enabled: member.isActive, no_sk: skNumber,
                jenis_patroli: jenisPatroli, id_daerah_patroli: formData.id_daerah_patroli,
                new_user: true,
                id_regu_tim_patroli: '',
                new_posko: false
            };
            apiCalls.push(modifySk(payload));
        }
        try {
            await Promise.all(apiCalls);
            await Swal.fire({
                title: 'Berhasil!', text: 'Regu baru telah berhasil ditambahkan.', icon: 'success',
                timer: 2000, showConfirmButton: false, timerProgressBar: true,
            });
            onSuccess();
            handleClose();
        } catch (err: any) {
            console.error('Gagal menambah regu baru:', err);
            Swal.fire({
    ...
    const renderMemberRow = (memberData: SuratTugasTeamMemberData, emailOption: EmailOption | null, onEmailChange: (opt: any) => void, onRoleChange: (value: string) => void, onActiveChange: (value: boolean) => void) => (
        <tr className={memberData.r_role_id === '4' ? 'bg-blue-50' : ''}>
            ...
            <td className="px-4 py-2 min-w-[300px]">
                <AsyncSelect cacheOptions defaultOptions loadOptions={loadUserOptions} value={emailOption} onChange={onEmailChange}
            ...
            <div className={`bg-white rounded-xl shadow-xl w-11/12 max-w-7xl max-h-[90vh] overflow-y-auto transition-transform duration-300 transform ${isVisible ? 'translate-y-0 scale-100' : 'translate-y-10 scale-95'}`}>
                ...
                <div className="p-6">
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Nomor Surat Tugas</label><input type="text" value={skNumber} disabled className="bg-gray-100 w-full sm:text-sm border-gray-300 rounded-md p-2" />
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Desa/Posko</label><AsyncSelect<KelurahanOption> cacheOptions defaultOptions loadOptions={promiseOptions} value={selectedKelurahan} onChange={handleKelurahanSelect} isClearable placeholder="Ketik min. 3 karakter..." styles={kelurahanSelectStyles} menuPortalTarget={document.body} menuPosition="fixed" noOptionsMessage={({ inputValue }) => inputValue.length < 3 ? 'Ketik minimal 3 karakter' : `Tidak ada hasil untuk "${inputValue}"`} />
                            </div>
                            <div>
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Jenis Patroli</label>
                                <select value={jenisPatroli} onChange={(e) => setJenisPatroli(e.target.value)} className="w-full sm:text-sm border-gray-300 rounded-md p-2" required>
                                    <option value="mandiri">Mandiri</option><option value="rutin">Rutin</option>
                                    <option value="terpadu">Terpadu</option><option value="pemadaman">Pemadaman</option>
                                </select>
                            </div>
                        </div>
                        <div>
