...
const InfoWindowContent: React.FC<InfoWindowContentProps> = ({ patroli, isLoggedin, apiV2URL, onClose }) => {
    ...
    return (
        <div className="font-sans w-full max-w-[280px] p-0 m-0 rounded-lg overflow-hidden shadow-md bg-white text-sm">
            <div
            ...
            >
                ...
                <div
                    onClick={onClose}
                    className="close-button w-6 h-6 rounded-full bg-white flex items-center justify-center cursor-pointer ml-2 border-2 border-red-500 box-border flex-shrink-0"
                >
                    <svg
                        width="14"
                        height="14"
                ...
                <div className="grid gap-2.5">
                    ...
                    {isLoggedin && patroli.kategori_patroli === 'Pemadaman' && (
                        <InfoRow
                            ...
                            value={
                                <span
                                    className={`inline-block px-1.5 py-0.5 rounded text-xs ${patroli.status_padam === 'Selesai'
...
const InfoRow: React.FC<InfoRowProps> = ({ label, value }) => (
    <div className="flex items-start">
        <div className="w-[70px] text-xs text-gray-600 font-medium flex-shrink-0 pt-0.5">
            {label}
        </div>
...
const ActionButton: React.FC<ActionButtonProps> = ({ href, icon, text, bgColorClass, textColorClass }) => {
    ...
    return (
        <a
            ...
            target="_blank"
            rel="noreferrer"
            className={`inline-flex items-center justify-center gap-1 px-2.5 py-1.5 rounded ${bgColorClass} ${textColorClass} no-underline text-xs font-medium transition-colors hover:bg-opacity-90 min-w-[100px] text-center`}
            style={{
